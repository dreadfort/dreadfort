...
with patch('dreadfort.api.tenant.resources.tenant_util', MagicMock()):
    from dreadfort.api.tenant.resources import EventProducerResource
    from dreadfort.api.tenant.resources import EventProducersResource
    from dreadfort.api.tenant.resources import MESSAGE_TOKEN
    from dreadfort.api.tenant.resources import TenantResource
    from dreadfort.api.tenant.resources import TokenResource
    from dreadfort.api.tenant.resources import UserResource
from dreadfort.data.model.tenant import EventProducer
...
class TestingTenantResourceOnPost(TenantApiTestBase):
    ...
    def test_return_400_for_tenant_id_empty(self):
        with patch('dreadfort.api.tenant.resources.tenant_util.find_tenant',
    ...
    def test_return_200_for_tenant_created(self):
        with patch('dreadfort.api.tenant.resources.tenant_util.find_tenant',
                ...
                'dreadfort.api.tenant.resources.tenant_util.create_tenant',
                MagicMock()):
            self.simulate_request(
                self.test_route,
                method='POST',
...
class TestingEventProducersResourceOnPost(TenantApiTestBase):
    ...
    def test_return_201_for_one_supported_sink(self):
        ...
        with patch(
                'dreadfort.api.tenant.resources.tenant_util.find_tenant',
                self.tenant_found), \
            patch(
                'dreadfort.api.tenant.resources.tenant_util.save_tenant',
                save_tenant), \
            patch(
                'dreadfort.api.tenant.resources.'
...
class TestingEventProducerResourceOnDelete(TenantApiTestBase):
    ...
    def test_should_return_200_on_get(self):
        ...
        with patch('dreadfort.api.tenant.resources.tenant_util',
                   mock_tenant_util):
            self.simulate_request(
                '/v1/tenant/{tenant_id}/producers/{event_producer_id}'.format(
                    tenant_id=self.tenant_id,
